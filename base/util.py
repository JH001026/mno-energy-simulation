...
def mno_to_id(mno):
    return settings.mnos.index(mno)


def id_to_mno(mno_id):
    return settings.mnos[mno_id]
...
def to_radio_type_string(radio):
    if radio == settings.RADIO_LTE:
        return "LTE"
    if radio == settings.RADIO_UMTS:
        return "3G"
    if radio == settings.RADIO_5G_NR:
        return '5G'
    if radio == settings.RADIO_GSM:
        return "GSM"


def zip_code_to_municipality():
    ...
    return {key: code[value] for key, value in municipality_code.items()}
...
def get_province(point):
    for i in range(len(settings.regions)):
        region = settings.regions[i]
        if region.contains(point).any():
            return settings.provinces[i]
