...
def find_links(p):
    ...
    if interf_loss is None:
        ...
        cuda_find_user_links_setup()

        # kernel_find_bs_close(...)
        # We have a the 10 closest Base Station IDs for each user
...
def cuda_find_user_links_setup():
    pass



@cuda.jit
def kernel_find_user_links(U, B):
    """
    :param U Grid of user data. Format: [[id, x, y], [id, x, y], ...]
    :param B Grid of base station data. Format:
    [[id, x, y, [channel_1, channel_2, ... ]],
    [id, x, y, [channel_1, channel_2, ... ]], ...]
    """
    pos = cuda.grid(1)
    if pos < U.size:
